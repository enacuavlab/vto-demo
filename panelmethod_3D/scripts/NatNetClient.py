...
class NatNetClient:
    ...
    def __unpack_rigid_body(self, data, major, minor, rb_num):
        ...
        trace_mf("RB: {rb_num:3.1d} ID: f{new_id:3.1d}")
    ...
    def __unpack_marker_set_data(self, data, packet_size, major, minor):
        ...
        for _ in range(0, marker_set_count):
            ...
            try:
                modelname = model_name.decode("utf-8")
                trace_mf("Model Name      : ", modelname)
            except UnicodeDecodeError:
                modelname = model_name.decode("windows-1252")
                trace_mf("Model Name      : ", modelname)
    ...
    def __unpack_frame_suffix_data(self, data, packet_size, major, minor):
        ...
        if (packet_size - offset) <= 0:
            ...
            if major >= 4:
                ...
                out_string = "Precision timestamp (h:m:s) - %4.1d:%2.2d:%2.2d" % (
                    hours,
                    minutes,
                    seconds,
                )
    ...
    def __unpack_mocap_data(self, data: bytes, packet_size, major, minor):
        ...
        if self.rigid_body_marker_set_list_listener is not None:
            self.rigid_body_marker_set_list_listener(
                rigid_body_data, marker_set_data, timestamp
            )
    ...
    def __unpack_rigid_body_description(self, data, major, minor):
        ...
        if (major >= 3) or (major == 0):
            ...
            for marker in marker_count_range:
                ...
                trace_dd(
                    "\t%3.1d Marker Label: %s Position: [x=%3.2f,y=%3.2f,z=%3.2f] %s"
                    % (
                        marker,
                        active_label,
                        marker_offset[0],
    ...
    def __unpack_asset_marker_data(self, data, major, minor):
        ...
        marker_data = DataDescriptions.AssetMarkerData(
